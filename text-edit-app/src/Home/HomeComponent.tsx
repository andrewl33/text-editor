...
export default class HomeComponent extends React.Component<HomeProps, HomeComponentState> {
  ...
  public render() {
    ...
    const { textLoading, collectionLoading, logInLoading, accountName, password } = this.state;
    
    const loggedInComponent = 
      this.props.loggedIn 
       ? ( 
        <Button fluid={true} primary={true} size='huge' onClick={this.props.toDashboard}>
          Go To Dashboard
        </Button>
       )
       : (
        <Form inverted={true}>
          <Form.Field>
            ...
            <input placeholder='Account Name' value={accountName} onChange={this.handleAccountNameChange}/>
          </Form.Field>
          <Form.Field>
            <label>Password</label>
            <input placeholder='Password' value={password} onChange={this.handlePasswordChange}/>
          </Form.Field>
          <br />
          <Button fluid={true} primary={true} type='submit' size='huge'onClick={this.onLogIn}>
            <Icon name={!logInLoading?'user outline':'circle notch'} loading={logInLoading}/>
            Log in
          </Button>
        </Form>
       );
      ...
      <Container style={{height: '100%'}}>
        <Grid textAlign='center' verticalAlign='middle' centered={true} style={{height: '100%'}}>
          <Grid.Column textAlign='center' centered={true} style={{ maxWidth: 450 }}>
            ...
            />
            <Segment inverted={true} textAlign='center' style={{ maxWidth: 450 }} padded="very">
              <Button fluid={true} primary={true} size='huge' onClick={this.newFile}>
                <Icon name={!textLoading?'file outline':'circle notch'} loading={textLoading} />
                New File
              </Button>
              <br />
              <Button fluid={true} primary={true} size='huge' onClick={this.newCollection}>
                <Icon name={!collectionLoading?'folder outline':'circle notch'} loading={collectionLoading}/>
                New Collection
              </Button>
              <br />
              {loggedInComponent}
  ...
  private handleAccountNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
