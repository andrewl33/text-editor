...
export default class SidebarComponent extends React.Component<SidebarProps, SidebarState> {
  ...
  public render() {
    ...
    const { pageType, name, createDate, users, isPrivate, onRemoveTag } = this.props;
    const nameField = 
      !nameEdit ? (
        <span>
          {name}
        ...
        <span>
          <Input placeholder="New name" value={nameInput} onChange={this.onNameInputChange}/>
    ...
    const tagField = 
      tagEdit ? (
        <span>
          <Input placeholder="New tag" value={tagType} onChange={this.onTagInputChange} />
    ...
    if (pageType === 'file') {
      ...
      tags = tagList && tagList.map((tag: string, idx: number) => {
        return (
          <Label key={idx} link={true} onClick={onRemoveTag && onRemoveTag.bind(tag)}>
            {tag}
            <Icon name='close' />
          </Label>
        );
    ...
    const usersElement = users && users.map((user: string, idx: number) => {
      return (
        <Label key={idx}>
          {user}
        </Label>
